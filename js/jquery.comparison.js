		   ...
		   // if the button has the class active we have items to compare
	       	if($(this).hasClass('active')){
				...
				for (var i = 0; i < arrayLength; i++) {
					...
					$('.modal-products').append('<div class="product"><img src="'+$(product).find('img').attr('src')+'"/><ul><li><span>'+$(product).data('title')+'</span></li><li><span>'+$(product).data('description')+'</span></li><li><span>'+$(product).data('price')+'</span></li><li><span>'+$(product).data('ingredients')+'</span></li><li><span>Origin: '+$(product).data('country')+'</span></li></ul></div>');
